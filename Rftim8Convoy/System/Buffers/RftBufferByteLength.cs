...
{
    public class RftBufferByteLength
    {
        private const string formatter = "{0,10}{1,20}{2,9}{3,12}";

        public RftBufferByteLength()
        {
            ...
            Console.WriteLine(formatter, "Array name", "Array type", "Length", "ByteLength");
        ...
        private static void ArrayInfo(Array arr, string name)
        {
            int byteLength = Buffer.ByteLength(arr);

            // Display the array name, type, Length, and ByteLength.
            Console.WriteLine(formatter, name, arr.GetType(), arr.Length, byteLength);
        }
    }
}
