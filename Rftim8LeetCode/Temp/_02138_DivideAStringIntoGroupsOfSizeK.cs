...
{
    public class _02138_DivideAStringIntoGroupsOfSizeK
    {
        /// <summary>
        ...
        {
            string[] x = DivideString("abcdefghi", 3, 'x');
            RftTerminal.RftReadResult(x);
            string[] x0 = DivideString("abcdefghij", 3, 'x');
        ...
        private static string[] DivideString(string s, int k, char fill)
        {
            ...
            {
                string t = s[^rem..];
                t = $"{t}{string.Concat(Enumerable.Repeat(fill, k - t.Length))}";
                r[^1] = t;
            }
