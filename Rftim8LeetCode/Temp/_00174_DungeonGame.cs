namespace Rftim8LeetCode.Temp
{
    public class _00174_DungeonGame
    {
        ...
        /// </summary>
        public _00174_DungeonGame()
        {
            Console.WriteLine(CalculateMinimumHP(
            [
                [-2, -3, 3],
                [-5, -10, 1],
                [10, 30, -5]
        ...
        private static int CalculateMinimumHP(int[][] dungeon)
        {
            ...
            int[] r = Enumerable.Repeat(int.MaxValue, m + 1).ToArray();
            r[m - 1] = 1;
            for (int i = n - 1; i >= 0; i--)
            {
                for (int j = m - 1; j >= 0; j--)
                {
                    r[j] = Math.Max(1, Math.Min(r[j], r[j + 1]) - dungeon[i][j]);
                }
            }
