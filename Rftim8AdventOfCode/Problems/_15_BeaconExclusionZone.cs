...
{
    public class _15_BeaconExclusionZone : I_15_BeaconExclusionZone
    {
        #region Static
        private readonly List<string>? data;

        public _15_BeaconExclusionZone()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_15_BeaconExclusionZone));
        }
        ...
        private static long PartOne0(List<string> input)
        {
            ...
            (HashSet<(long X, long Y)> map, List<Sensor> sensors) = FormatInput(input);

            long rangeMin = sensors.Select(s => s.X - (s.BeaconDistance - Math.Abs(s.Y - rowToCheck))).Min();
            long rangeMax = sensors.Select(s => s.X + (s.BeaconDistance - Math.Abs(s.Y - rowToCheck))).Max();
        ...
        private static long PartTwo0(List<string> input)
        {
            ...
            (HashSet<(long X, long Y)> _, List<Sensor> sensors) = FormatInput(input);

            foreach (Sensor sensor in sensors)
            {
                for (long i = Math.Max(0, sensor.X - sensor.BeaconDistance - 1); i <= Math.Min(maxCoordinate, sensor.X + sensor.BeaconDistance + 1); ++i)
                {
                    long positiveY = Math.Min(sensor.Y + sensor.BeaconDistance + 1 - Math.Abs(sensor.X - i), maxCoordinate);
                    long negativeY = Math.Max(sensor.Y - (sensor.BeaconDistance + 1 - Math.Abs(sensor.X - i)), 0);

                    if (sensors.All(s => Math.Abs(s.X - i) + Math.Abs(s.Y - positiveY) > s.BeaconDistance)) return i * 4000000 + positiveY;
                    if (sensors.All(s => Math.Abs(s.X - i) + Math.Abs(s.Y - negativeY) > s.BeaconDistance)) return i * 4000000 + negativeY;
                }
            }
        ...
        private static (HashSet<(long X, long Y)> Map, List<Sensor> Sensors) FormatInput(List<string> input)
        {
            HashSet<(long X, long Y)> map = [];
            List<Sensor> sensors = [];
            foreach ((string[] split, (long, long) beacon) in from string line in input
                                                              let split = line.Replace(",", "").Replace(":", "").Split(' ')
                                                              let beacon = (long.Parse(split[8].Split('=')[1]), long.Parse(split[9].Split('=')[1]))
                                                              select (split, beacon))
            {
                (long sensorX, long sensorY) = (long.Parse(split[2].Split('=')[1]), long.Parse(split[3].Split('=')[1]));
                Sensor sensor = new(sensorX, sensorY, Math.Abs(sensorX - beacon.Item1) + Math.Abs(sensorY - beacon.Item2));
                map.Add(beacon);
        ...
        private record Sensor(long X, long Y, long BeaconDistance);
        #endregion
        ...
        public _15_BeaconExclusionZone(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_15_BeaconExclusionZone));
        }
