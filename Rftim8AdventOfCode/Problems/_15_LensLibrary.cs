...
{
    public class _15_LensLibrary : I_15_LensLibrary
    {
        #region Static
        private readonly List<string>? data;

        public _15_LensLibrary()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_15_LensLibrary));
        }
        ...
        /// loose-fitting hard hat.The reindeer leads you to a shelf of goggles and hard hats(you quickly find some that fit) and 
        /// then further into the facility.At one point, you pass a button with a faint snout mark and the label "PUSH FOR HELP". 
        ...
        /// To verify that your HASH algorithm is working, the book offers the sum of the result of running the HASH algorithm 
        /// on each step in the initialization sequence.
        /// 
        /// For example:
        /// 
        /// rn= 1, cm-, qp= 3, cm= 2, qp-, pc= 4, ot= 9, ab= 5, pc-, pc= 6, ot= 7
        /// This initialization sequence specifies 11 individual steps; the result of running the HASH algorithm on each 
        ...
        /// 
        /// The label will be immediately followed by a character that indicates the operation to perform: either an 
        ...
        /// it is present in the box.Then, move any remaining lenses as far forward in the box as they can go without changing 
        /// their order, filling any space made by removing the indicated lens. (If no lens in that box has the given label, nothing happens.)
        /// 
        ...
        private static int PartTwo0(List<string> input)
        {
            ...
            foreach (string item in x)
            {
                if (item.Contains('='))
                {
                    ...
                    string t1 = $"{t} {t0}";
                    if (!kvp.TryGetValue(r, out List<string>? value)) kvp.Add(r, [t1]);
                    else
                    {
                        bool f = false;
                        for (int i = 0; i < value.Count; i++)
                        {
                            if (value[i].Split(" ")[0] == t)
                            {
                                value[i] = t1;
                                f = true;
                            }
                ...
                else
                {
                    ...
                    if (kvp.TryGetValue(r, out List<string>? value))
                    {
                        for (int i = 0; i < value.Count; i++)
                        {
                            if (value[i].Split(" ")[0] == t)
            ...
            foreach (KeyValuePair<int, List<string>> item in kvp.Where(o => o.Value.Count > 0))
            {
                for (int i = 0; i < item.Value.Count; i++)
                {
                    int r = (item.Key + 1) * (i + 1) * int.Parse(item.Value[i].Split(" ")[1]);
                    y.Add(r);
                }
            }
        ...
        public _15_LensLibrary(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_15_LensLibrary));
        }
