...
{
    public class _09_EncodingError : I_09_EncodingError
    {
        #region Static
        private readonly List<string>? data;

        public _09_EncodingError()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_09_EncodingError));
        }
        ...
        /// XMAS starts by transmitting a preamble of 25 numbers.After that, each number you receive should be the sum 
        ...
        {
            List<long> numbers = input.Select(long.Parse).ToList();
            long invalidNumber = PartOne0(input);
            ...
            while (true)
            {
                ...
                {
                    List<long> range = numbers.Skip(currentIdx).Take(--setSize).ToList();
        ...
        public _09_EncodingError(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_09_EncodingError));
        }
