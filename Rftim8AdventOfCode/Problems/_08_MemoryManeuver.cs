...
{
    public class _08_MemoryManeuver : I_08_MemoryManeuver
    {
        #region Static
        private readonly List<string>? data;

        public _08_MemoryManeuver()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_08_MemoryManeuver));
        }
        ...
        /// If a referenced child node does not exist, that reference is skipped.A child node can be referenced multiple time and counts each time it is referenced.
        ...
        private static Node ReadNode(List<int> numbers, ref int i)
        {
            Node node = new();
        ...
        private class Node
        {
            ...
            public int Value()
            {
                if (Nodes.Count == 0) return Metadata.Sum();

                int value = 0;

                foreach (var m in Metadata)
                {
                    if (m <= Nodes.Count) value += Nodes[m - 1].Value();
                }
        ...
        public _08_MemoryManeuver(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_08_MemoryManeuver));
        }
