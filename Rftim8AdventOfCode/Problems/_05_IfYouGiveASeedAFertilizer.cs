...
{
    public class _05_IfYouGiveASeedAFertilizer : I_05_IfYouGiveASeedAFertilizer
    {
        #region Static
        private readonly List<string>? data;

        public _05_IfYouGiveASeedAFertilizer()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_05_IfYouGiveASeedAFertilizer));
        }
        ...
        private static long PartOne0(List<string> input)
        {
            int n = input.Count;
            List<long> r = [];
            List<string> seeds = [.. input[0].Split(":")[1].Split(" ", StringSplitOptions.RemoveEmptyEntries)];
            foreach (string item in seeds)
            {
                long c = long.Parse(item);
                bool f = false;
                for (int i = 2; i < n; i++)
                {
                    ...
                    {
                        a1 = input[i].Split("-")[0];
                        a2 = input[i].Split("-")[2].Split(" ")[0];
                    }
                    else if (!string.IsNullOrEmpty(input[i]))
                    {
                        if (f == false)
                        {
                            List<long> q = [.. input[i].Split(" ").Select(long.Parse)];
        ...
        private static long PartTwo0(List<string> input)
        {
            ...
            // Linq
            List<long> seeds = [.. input[0].Split(":")[1].Split(" ", StringSplitOptions.RemoveEmptyEntries).Select(long.Parse)];
            ...
            {
                if (string.IsNullOrEmpty(item) && t.Count != 0)
                {
                    maps.Add(t);
                    t = [];
                }
                else if (!item.Contains("seeds") && !item.Contains("map") && !string.IsNullOrEmpty(item)) t.Add(item);
            }
            if (t.Count > 0) maps.Add(t);
            ...
            foreach (List<string> map in maps)
            {
                ...
                {
                    List<long> q = [.. item.Split(" ").Select(long.Parse)];
                    long seed = q[0];
                    long soil = q[1];
                    long rlen = q[2];
                    ...
                    {
                        (long item1, long item2) = ranges[i];

                        if (soil <= item1 && item1 < soil + rlen && soil + rlen <= item1 + item2)
                        {
                            r.Add((item1 - soil + seed, soil + rlen - item1));
                            ranges[i] = (soil + rlen, item1 + item2 - soil - rlen);
                        }
                        else if (item1 <= soil && soil < item1 + item2 && item1 + item2 <= soil + rlen)
                        {
                            r.Add((seed, item1 + item2 - soil));
                            ranges[i] = (item1, soil - item1);
                        }
                        else if (item1 <= soil && soil < soil + rlen && soil + rlen <= item1 + item2)
                        {
                            r.Add((seed, rlen));
                            ranges[i] = (item1, soil - item1);
                            ranges.Add((soil + rlen, item1 + item2 - soil - rlen));
                        }
                        if (soil <= item1 && item1 < item1 + item2 && item1 + item2 <= soil + rlen)
                        {
                            r.Add((item1 - soil + seed, item2));
                            ranges[i] = ranges[^1];
            ...
            return ranges.MinBy(o => o.Item1).Item1;
        }
        #endregion
        ...
        public _05_IfYouGiveASeedAFertilizer(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_05_IfYouGiveASeedAFertilizer));
        }
