...
{
    public class _25_LetItSnow : I_25_LetItSnow
    {
        #region Static
        private readonly List<string>? data;

        public _25_LetItSnow()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_25_LetItSnow));
        }
        ...
        {
            List<string> l = [.. input[0].Replace(",", "").Replace(".", "").Split(' ')];
            List<int> rc = [];

            foreach (string item in l)
            {
                if (int.TryParse(item, out int value)) rc.Add(value);
            }

            int max = Math.Max(rc[0], rc[1]) * 2;
            long[,] x = new long[max + 1, max + 1];
            long t = 20151125;
            x[0, 0] = t;
        ...
        public _25_LetItSnow(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_25_LetItSnow));
        }
