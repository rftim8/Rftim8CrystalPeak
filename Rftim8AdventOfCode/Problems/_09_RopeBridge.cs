...
{
    public class _09_RopeBridge : I_09_RopeBridge
    {
        #region Static
        private readonly List<string>? data;

        public _09_RopeBridge()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_09_RopeBridge));
        }
        ...
        {
            IEnumerable<Command> commands = input.Select(c => new Command(c));
            const int knotCount = 10;
            (int X, int Y)[] knots = new (int X, int Y)[knotCount];
            HashSet<(int X, int Y)> visited = [];
            HashSet<(int X, int Y)> visited10 = [];

            foreach (Command command in commands)
            {
                for (int i = 0; i < command.Moves; i++)
                {
                    ...
                    {
                        int distanceX = knots[k - 1].X - knots[k].X;
                        int distanceY = knots[k - 1].Y - knots[k].Y;
        ...
        private static int PartTwo0(List<string> input)
        {
            ...
            return visited10.Count;
        }

        record Command((int X, int Y) Direction, int Moves)
        {
            public Command(string line) : this(default, default)
            {
        ...
        public _09_RopeBridge(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_09_RopeBridge));
        }
