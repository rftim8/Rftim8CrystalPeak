...
{
    public class _05_PrintQueue : I_05_PrintQueue
    {
        #region Static
        private readonly List<string>? data;

        public _05_PrintQueue()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_05_PrintQueue));
        }
            ...
            foreach (string item in c)
            {
                ...
                for (int j = 0; j < d.Count; j++)
                {
                    for (int k = j + 1; k < d.Count; k++)
                    {
                        ...
                        {
                            string x = item1.Split('|')[0];
                            string y = item1.Split('|')[1];
        ...
        public _05_PrintQueue(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_05_PrintQueue));
        }
