...
{
    public class _24_ArithmeticLogicUnit : I_24_ArithmeticLogicUnit
    {
        #region Static
        private readonly List<string>? data;

        public _24_ArithmeticLogicUnit()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_24_ArithmeticLogicUnit));
        }

        /// <summary>
        /// Magic smoke starts leaking from the submarine's arithmetic logic unit (ALU). Without the ability to perform basic arithmetic and logic functions,
        ...
        /// In all of these instructions, a and b are placeholders; a will always be the variable where the result of the operation is stored(one of w, x, y, or z), 
        ...
        /// 
        /// (Program authors should be especially cautious; attempting to execute div with b = 0 or attempting to execute mod with a<0 or b<=0 will cause the program 
        /// to crash and might even damage the ALU.These operations are never intended in any serious ALU program.)
        /// 
        ...
        /// each expecting a single digit of the model number in order of most to least significant. (So, to check the model number 13579246899999, 
        ...
        {
            (int a, int b, int c)[] sets = input.Chunk(18)
                 .SelectMany(c => new[] { c[4], c[5], c[15] })
                 .Select(c => Convert.ToInt32(c[6..]))
                 .Chunk(3)
            ...
            {
                if (digits.Length == sets.Length) return z == 0 ? [long.Parse(string.Join("", digits))] : Array.Empty<long>();

                IEnumerable<long> sub(IEnumerable<int> range, Func<int, int> z) => range.SelectMany(w => check([.. digits, w], i + 1, z(w)));
            ...
            IEnumerable<long> serials = check([], 0, 0);
        ...
        public _24_ArithmeticLogicUnit(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_24_ArithmeticLogicUnit));
        }
