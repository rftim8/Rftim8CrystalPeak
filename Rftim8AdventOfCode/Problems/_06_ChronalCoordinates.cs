...
{
    public class _06_ChronalCoordinates : I_06_ChronalCoordinates
    {
        #region Static
        private readonly List<string>? data;

        public _06_ChronalCoordinates()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_06_ChronalCoordinates));
        }
        ...
        /// 
        /// Using only the Manhattan distance, determine the area around each coordinate by counting the number of integer X, 
        /// Y locations that are closest to that coordinate (and aren't tied in distance to any other coordinate).
        /// 
        ...
        {
            IEnumerable<(int x, int y)> coordinates = input
                 .Select(x => x.Split(", "))
                 .Select(x => (x: int.Parse(x[0]), y: int.Parse(x[1])));

            IEnumerable<int> getAreas(int start, int end) =>
                from x in Enumerable.Range(start, end - start)
                from y in Enumerable.Range(start, end - start)
                let distanceGroups = from coordinate in coordinates
                                     let distance = Math.Abs(coordinate.x - x) + Math.Abs(coordinate.y - y)
                                     group coordinate by distance
                let closestDistanceGroup = distanceGroups.OrderBy(x => x.Key).First()
                where closestDistanceGroup.Count() == 1
                let closestCoordinate = closestDistanceGroup.First()
                group closestCoordinate by closestCoordinate into closestCoordinates
                orderby closestCoordinates.Key
                select closestCoordinates.Count();

            int[] areas1 = getAreas(0, 500).ToArray();
            int[] areas2 = getAreas(-1, 501).ToArray();

            return areas1.Zip(areas2, (a1, a2) => a1 == a2 ? a1 : 0)
        ...
        private static int PartTwo0(List<string> input)
        {
            ...
            IEnumerable<(int x, int y)> area =
                from x in Enumerable.Range(0, 1000)
                from y in Enumerable.Range(0, 1000)
                let distances =
                    from coordinate in coordinates
                    select Math.Abs(coordinate.x - x) + Math.Abs(coordinate.y - y)
                where distances.Sum() < 10000
                select (x, y);
        ...
        public _06_ChronalCoordinates(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_06_ChronalCoordinates));
        }
