...
{
    public class _22_ReactorReboot : I_22_ReactorReboot
    {
        #region Static
        private readonly List<string>? data;

        public _22_ReactorReboot()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_22_ReactorReboot));
        }
        ...
        /// Each step specifies a cuboid(the set of all cubes that have coordinates which fall within ranges for x, y, and z) and whether to turn all of the cubes 
            ...
            foreach (Cube cube in input)
            {
                for (int x = Math.Max(cube.XMin, -50); x <= Math.Min(cube.XMax, 50); x++)
                {
                    for (int y = Math.Max(cube.YMin, -50); y <= Math.Min(cube.YMax, 50); y++)
                    {
                        ...
                        {
                            if (cube.On) onCubes.Add((x, y, z));
                            else if (onCubes.Contains((x, y, z))) onCubes.Remove((x, y, z));
                        }
                    }
                }
            }
        ...
        private static long PartTwo0(List<string> input0)
        {
            ...
            {
                if (a.XMin > b.XMax || a.XMax < b.XMin || a.YMin > b.YMax || a.YMax < b.YMin || a.ZMin > b.ZMax || a.ZMax < b.ZMin) return null;
            ...
            List<Cube> resultingCubes = [];
            foreach (Cube cube in input)
            {
                List<Cube> cubesToAdd = [];
                if (cube.On) cubesToAdd.Add(cube);

                foreach (Cube otherCube in resultingCubes)
                {
                    Cube? intersection = Intersect(cube, otherCube, !otherCube.On);

                    if (intersection != null) cubesToAdd.Add(intersection.Value);
                }
            ...
            return resultingCubes.Aggregate(0L, (totalVolume, c) => totalVolume + Volume(c) * (c.On ? 1 : -1));
        }

        private static readonly char[] separator = [',', '.', ' ', '=', 'x', 'y', 'z'];

        private record struct Cube(bool On, int XMin, int XMax, int YMin, int YMax, int ZMin, int ZMax);
        #endregion
        ...
        public _22_ReactorReboot(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_22_ReactorReboot));
        }
