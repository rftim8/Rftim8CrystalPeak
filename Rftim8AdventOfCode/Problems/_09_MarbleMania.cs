...
{
    public class _09_MarbleMania : I_09_MarbleMania
    {
        #region Static
        private readonly List<string>? data;

        public _09_MarbleMania()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_09_MarbleMania));
        }
        ...
        /// it is still a circle: the marble is both clockwise from itself and counter-clockwise from itself.This marble is designated the current marble.
        /// 
        ...
        {
            int players = int.Parse(input[0].Split(' ')[0]);
            int last = int.Parse(input[0].Split(' ')[6]);

            long[] scores = new long[players];
            LinkedList<long> circle = new();
            LinkedListNode<long>? current = circle.AddFirst(0);
            ...
            for (int i = 1; i < last; i++)
            {
                ...
                else current = circle.AddAfter(current?.Next ?? circle.First!, i);
            }
        ...
        {
            int players = int.Parse(input[0].Split(' ')[0]);
            int last = int.Parse(input[0].Split(' ')[6]) * 100;
        ...
        public _09_MarbleMania(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_09_MarbleMania));
        }
