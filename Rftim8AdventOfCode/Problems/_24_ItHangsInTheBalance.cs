...
{
    public class _24_ItHangsInTheBalance : I_24_ItHangsInTheBalance
    {
        #region Static
        private readonly List<string>? data;

        public _24_ItHangsInTheBalance()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_24_ItHangsInTheBalance));
        }
        ...
        /// 
        /// Of course, that's not the only problem. The first group - the one going in the passenger compartment - needs as few packages as possible so that 
        /// Santa has some legroom left over. It doesn't matter how many packages are in either of the other two groups, so long as all of the groups weigh the same.
        /// 
        ...
        /// in the passenger compartment gives Santa the most legroom and wins.In this situation, the quantum entanglement for the ideal configuration is therefore 99.
        ...
        {
            int[] x = input.Select(int.Parse).ToArray();
            int totalWeight = x.Sum();
            long minQe = Split(x, 0, totalWeight / 3, 1, 0).Item1;
        ...
        private static long PartTwo0(List<string> input)
        {
            int[] x = input.Select(int.Parse).ToArray();
            int totalWeight = x.Sum();
            long minQe = Split(x, 0, totalWeight / 4, 1, 0).Item1;
        ...
        {
            if (remWt == 0) return new Tuple<long, int>(currQe, currCt);
            else if (remWt < 0 || pos == weights.Length) return new Tuple<long, int>(long.MaxValue, int.MaxValue);

            Tuple<long, int> included = Split(weights, pos + 1, remWt - weights[pos], currQe * weights[pos], currCt + 1);
            Tuple<long, int> notIncluded = Split(weights, pos + 1, remWt, currQe, currCt);

            if (included.Item2 < notIncluded.Item2) return included;
            else if (notIncluded.Item2 < included.Item2) return notIncluded;
            else return included.Item1 < notIncluded.Item1 ? included : notIncluded;
        }
        #endregion
        ...
        public _24_ItHangsInTheBalance(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_24_ItHangsInTheBalance));
        }
