...
{
    public class _12_PassagePathing : I_12_PassagePathing
    {
        #region Static
        private readonly List<string>? data;

        public _12_PassagePathing()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_12_PassagePathing));
        }
        ...
        /// twice(once on the way to cave d and a second time when returning from cave d), and since cave b is small, this is not allowed.
        ...
        {
            IEnumerable<string[]> edges = input.Select(s => s.Split('-'));
            ILookup<string, string> graph = edges.Union(edges.Select(e => e.Reverse().ToArray()))
                                                 .ToLookup(e => e[0], e => e[1]);

            string[][] GetPaths(string vertex, string[] visited, int max)
            {
                string[] path = [.. visited, vertex];
                if (vertex == "end") return [path];

                string[] lower = path.Where(v => char.IsLower(v[0]))
                                     .ToArray();
                string[] exclude = lower.GroupBy(v => v)
                    .Any(g => g.Count() == max) ? lower : ["start"];
            ...
            int paths(int max) => GetPaths("start", [], max).Length;
        ...
        public _12_PassagePathing(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_12_PassagePathing));
        }
