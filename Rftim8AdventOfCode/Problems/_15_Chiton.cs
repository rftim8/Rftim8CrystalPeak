...
{
    public class _15_Chiton : I_15_Chiton
    {
        #region Static
        ...
        public _15_Chiton()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_15_Chiton));
        }
        ...
        /// add up the risk levels of each position you enter(that is, don't count the risk level of your starting position unless you enter it; 
        ...
        /// all of its risk levels are 1 higher than the tile immediately up or left of it.However, risk levels above 9 wrap back around to 1.
        /// So, if your original map had some position with a risk level of 8, then that same position on each of the 25 total tiles would be as follows:
        /// 
        /// 8 9 1 2 3
        /// 9 1 2 3 4
        /// 1 2 3 4 5
        ...
        /// Because the full map is actually five times larger in both dimensions, that position appears a total of 25 times, once in each duplicated tile, 
        ...
        private static Node[][]? _matrix;
        private static Node[][]? _matrixPartOne;
        private static Node[][]? _matrixPartTwo;
        ...
        private static long Part2()
        {
            ...
            int height = _matrixPartOne.Length;
            int width = _matrixPartOne[0].Length;
            for (int y = 0; y < 5; y++)
            {
                ...
                for (int i = 0; i < height; i++)
                {
                    ...
                    for (int x = 0; x < 5; x++)
                    {
                        ...
                        {
                            int value = (_matrixPartOne[i][j].Value - 1 + y + x) % 9 + 1;
            ...
            return Dijkstra(0, 0) - 1;
        }

        private static long Dijkstra(int startingY, int startingX)
        {
            ...
            while (q.Count > 0)
            {
                ...
                IEnumerable<Node> neighbours = GetUnvisitedNeighbours(u.Y, u.X);

                foreach (Node neighbour in neighbours)
                {
                    long alt = u.Distance + neighbour.Value;
        ...
        private static List<Node> GetUnvisitedNeighbours(int y, int x)
        {
            List<Node> unvisitedNeighbours = [];

            if (y > 0 && _matrix![y - 1][x].NotInQ) unvisitedNeighbours.Add(_matrix[y - 1][x]);
            if (y < _matrix!.Length - 1 && !_matrix[y + 1][x].NotInQ) unvisitedNeighbours.Add(_matrix[y + 1][x]);
            if (x > 0 && !_matrix[y][x - 1].NotInQ) unvisitedNeighbours.Add(_matrix[y][x - 1]);
            if (x < _matrix[y].Length - 1 && !_matrix[y][x + 1].NotInQ) unvisitedNeighbours.Add(_matrix[y][x + 1]);
        ...
        public _15_Chiton(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_15_Chiton));
        }
