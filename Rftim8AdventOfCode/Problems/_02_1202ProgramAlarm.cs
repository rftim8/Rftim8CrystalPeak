...
{
    public class _02_1202ProgramAlarm : I_02_1202ProgramAlarm
    {
        #region Static
        private readonly List<string>? data;

        public _02_1202ProgramAlarm()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_02_1202ProgramAlarm));
        }
        ...
        {
            List<int> x = input[0].Split(',', StringSplitOptions.TrimEntries).Select(int.Parse).ToList();
            x[1] = 12;
            x[2] = 2;
        ...
        /// Each time you try a pair of inputs, make sure you first reset the computer's memory to the values in the program (your puzzle input) - in other words, 
            ...
            for (int i = 0; i < 100; i++)
            {
                for (int j = 0; j < 100; j++)
                {
                    ...
                    for (int k = 0; k < y.Count; k++)
                    {
                        ...
                        else if (y[k] == 99) break;
                    }

                    if (y[0] == 19690720) return 100 * i + j;
                }
            }
        ...
        public _02_1202ProgramAlarm(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_02_1202ProgramAlarm));
        }
