...
{
    public class _16_PacketDecoder : I_16_PacketDecoder
    {
        #region Static
        private readonly List<string>? data;

        public _16_PacketDecoder()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_16_PacketDecoder));
        }
        ...
        {
            long pos = 0, vers = 0;
            StringReader? reader = new(string.Join("",
                Convert.FromHexString(input[0])
                .Select(b => Convert.ToString(b, 2)
                .PadLeft(8, '0'))));
            bool readBit() => ReadInt(1, ref pos, reader) == 1;
            ...
            long readLiteral()
            {
                ...
                {
                    more = readBit();
                    parts.Add(ReadRaw(4, ref pos, reader));
                } while (more);
            ...
            Packet readPacket()
            {
                Header header = ReadHeader(ref pos, reader);
                vers += header.Version;

                if (header.Type == 4) return new Literal(header, readLiteral());

                bool byPacket = readBit();
                int length = ReadInt(byPacket ? 11 : 15, ref pos, reader);
                List<Packet> packets = [];

                if (byPacket) packets = Enumerable.Range(0, length).Select(i => readPacket()).ToList();
                else
                {
                    long end = pos + length;
                    do
        ...
        private static Header ReadHeader(ref long pos, StringReader? reader) => new(ReadInt(3, ref pos, reader), ReadInt(3, ref pos, reader));

        private static int ReadInt(int length, ref long pos, StringReader? reader) => Convert.ToInt32(ReadRaw(length, ref pos, reader), 2);

        private static string ReadRaw(int length, ref long pos, StringReader? reader)
        {
        ...
        private record Header(int Version, int Type);

        private record Packet(Header Head)
        {
            public virtual long Eval() => 0;
        }

        private record Literal(Header Head, long Value) : Packet(Head)
        {
            public override long Eval() => Value;
        }

        private record Operation(Header Head, Packet[] Subpackets) : Packet(Head)
        {
        ...
        public _16_PacketDecoder(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_16_PacketDecoder));
        }
