...
{
    public class _13_PointOfIncidence : I_13_PointOfIncidence
    {
        #region Static
        private readonly List<string>? data;

        public _13_PointOfIncidence()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_13_PointOfIncidence));
        }
        ...
        /// frames keeping them in place.The mirrors are extremely flat and shiny, and many of the fallen mirrors 
        /// have lodged into the ash at strange angles.Because the terrain is all one color, 
        ...
        /// row 1 doesn't need to match anything. The remaining rows match: row 2 matches row 7, row 3 matches row 6, and row 4 matches row 5.
        /// 
        ...
        /// In the above example, the first pattern's vertical line has 5 columns to its left and the second pattern's horizontal line
        ...
        {
            int r = 0;
            List<List<string>> l = BuildGrid(input);

            foreach (List<string> pattern in l)
            {
                r += SolveGrid([.. pattern], 0);
            }
        ...
        /// In this example, the first pattern's new horizontal line has 3 rows above it and the second pattern's new horizontal line
        ...
        private static int PartTwo0(List<string> input)
        {
            ...
            foreach (List<string> pattern in l)
            {
                r += SolveGrid([.. pattern], 1);
            }

            return r;
        }

        private static List<List<string>> BuildGrid(List<string> input)
        {
            List<List<string>> l = [];
            List<string> l0 = [];
        ...
        private static int SolveGrid(string[] grid, int allowedSmudges)
        {
            // Horizontal
            for (int reflectAfterCol = 0; reflectAfterCol < grid[0].Length - 1; reflectAfterCol++)
            {
                int foundSmudges = 0;
                for (int y = 0; y < grid.Length; y++)
                {
                    for (int deltaCol = 0; reflectAfterCol - deltaCol >= 0 && reflectAfterCol + deltaCol + 1 < grid[0].Length; deltaCol++)
                    {
                        if (grid[y][reflectAfterCol - deltaCol] != grid[y][reflectAfterCol + deltaCol + 1])
                        {
                            foundSmudges++;
                            if (foundSmudges > allowedSmudges) break;
                        }
                    }
                ...
                if (foundSmudges == allowedSmudges) return reflectAfterCol + 1;
            }

            // Vertical
            for (int reflectAfterRow = 0; reflectAfterRow < grid.Length - 1; reflectAfterRow++)
            {
                int foundSmudges = 0;
                for (int x = 0; x < grid[0].Length; x++)
                {
                    for (int deltaRow = 0; reflectAfterRow - deltaRow >= 0 && reflectAfterRow + deltaRow + 1 < grid.Length; deltaRow++)
                    {
                ...
                if (foundSmudges == allowedSmudges) return (reflectAfterRow + 1) * 100;
            }
        ...
        public _13_PointOfIncidence(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_13_PointOfIncidence));
        }
