...
{
    public class _04_Scratchcards : I_04_Scratchcards
    {
        #region Static
        private readonly List<string>? data;

        public _04_Scratchcards()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_04_Scratchcards));
        }
        ...
        private static int PartOne0(List<string> input)
        {
            ...
            {
                List<int> x = [.. item.Split("|")[0].Split(":")[1].Split(" ", StringSplitOptions.RemoveEmptyEntries).Select(int.Parse)];
                List<int> y = [.. item.Split("|")[1].Split(" ", StringSplitOptions.RemoveEmptyEntries).Select(int.Parse)];

                int a = 0;

                foreach (int item1 in x)
                {
                    if (y.Contains(item1)) a = a == 0 ? 1 : a *= 2;
                }
        ...
        /// Once all of the originals and copies have been processed, you end up with 1 instance of card 1, 2 instances of card 2, 4 instances of card 3, 8 instances 
        /// of card 4, 14 instances of card 5, and 1 instance of card 6. In total, this example pile of scratchcards causes you to ultimately have 30 scratchcards!
        /// 
        ...
        private static int PartTwo0(List<string> input)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                {
                    int c = i;
                    List<int> x = [.. input[i].Split("|")[0].Split(":")[1].Split(" ", StringSplitOptions.RemoveEmptyEntries).Select(int.Parse)];
                    List<int> y = [.. input[i].Split("|")[1].Split(" ", StringSplitOptions.RemoveEmptyEntries).Select(int.Parse)];
        ...
        public _04_Scratchcards(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_04_Scratchcards));
        }
