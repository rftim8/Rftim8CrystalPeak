...
{
    public partial class _19_MonsterMessages : I_19_MonsterMessages
    {
        #region Static
        private readonly List<string>? data;

        public _19_MonsterMessages()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_19_MonsterMessages));
        }
        ...
        /// They think their satellite has collected an image of a sea monster! Unfortunately, the connection to the satellite is having problems, 
        ...
        /// but bababa, aaabbb, and aaaabbb do not, producing the answer 2. The whole message must match all of rule 0; there can't be extra unmatched characters in the message.
        ...
        {
            string[] messages = input.SkipWhile(l => l != string.Empty).Skip(1).ToArray();
            Dictionary<int, string> rules = input.TakeWhile(l => l != string.Empty)
                .Select(l => l.Split(": "))
                .ToDictionary(a => int.Parse(a[0]), a => a[1]);

            string s = rules[0];
            while (MyRegex().IsMatch(s))
            {
                s = MyRegex().Replace(s, m => $"({rules[int.Parse(m.Value)]})");
            }

            Regex p1re = new($"^{s.Replace(" ", "").Replace("\"", "")}$");

            return messages.Count(p1re.IsMatch);
        }
        ...
        /// it might help to start by looking at which rules always match the same set of values and how those rules (especially rules 42 and 31) 
        ...
        private static int PartTwo0(List<string> input)
        {
            ...
            while (true)
            {
                KeyValuePair<int, string> simple = rules.FirstOrDefault(r => !MyRegex().IsMatch(r.Value) && r.Key != 42 && r.Key != 31);
                if (simple.Equals(default(KeyValuePair<int, string>))) break;

                foreach (int i in rules.Keys.ToArray())
                {
            ...
            Regex p2re = new($"^{rules[0].Replace("8", $"({rules[42]})+").Replace("11", $"(?<A>{rules[42]})+(?<-A> {rules[31]})+").Replace(" ", "")}$");

            return messages.Count(p2re.IsMatch);
        }
        ...
        public _19_MonsterMessages(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_19_MonsterMessages));
        }
