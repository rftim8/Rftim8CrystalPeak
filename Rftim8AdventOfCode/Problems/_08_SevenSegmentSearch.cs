...
{
    public class _08_SevenSegmentSearch : I_08_SevenSegmentSearch
    {
        #region Static
        ...
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_08_SevenSegmentSearch));
        }
        ...
        /// and then write down a single four digit output value(your puzzle input). Using the signal patterns, 
        ...
        {
            List<string> digits = input
                .Select(line => line.Split('|')
                .Select(side => side
                .Split(' ', StringSplitOptions.RemoveEmptyEntries)
                .Select(word => new string([.. word.OrderBy(c => c)])))
                .ToList())
                .Select(sides =>
                {
                    string[] left = [.. sides[0].OrderBy(word => word.Length)];
                    Dictionary<string, char> map = left
            ...
            .ToList();
            int p1 = digits.Sum(d => d.Count(c => "1478".Contains(c)));
        ...
        public _08_SevenSegmentSearch(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_08_SevenSegmentSearch));
        }
