...
{
    public class _05_SunnyWithAChanceOfAsteroids : I_05_SunnyWithAChanceOfAsteroids
    {
        #region Static
        private readonly List<string>? data;

        public _05_SunnyWithAChanceOfAsteroids()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_05_SunnyWithAChanceOfAsteroids));
        }
        ...
        /// that is, the opcode is the rightmost two digits of the first value in an instruction. Parameter modes are single digits, one per parameter, 
        /// read right-to-left from the opcode: the first parameter's mode is in the hundreds digit, the second parameter's mode is in the thousands digit, 
        ...
        /// its value is the value stored at address 4 (33). The second parameter, 3 in immediate mode, simply has value 3. The result of this operation, 33 * 3 = 99, 
        ...
        /// 
        /// It will then perform a series of diagnostic tests confirming that various parts of the Intcode computer, like parameter modes, function correctly.For each test,
        /// it will run an output instruction indicating how far the result of the test was from the expected value, where 0 means the test was successful.
        ...
        /// </summary>
        [Benchmark]
        public int PartOne() => PartOne0(data!, 1);

        private static int PartOne0(List<string> input, int inputValue)
        {
            int r = 0;
            List<int> x = input[0].Split(',').Select(int.Parse).ToList();
            ...
            while (i < x.Count)
            {
                ...
                int param1 = 0;
                int param2 = 0;
                Mode param1Mode = Mode.Position;
                Mode param2Mode = Mode.Position;
                int instLength = 0;
                string currentCode = x[i].ToString();
                ...
                if (opCode != 3) param1 = param1Mode == Mode.Position ? x[x[i + 1]] : x[i + 1];

                if (opCode != 3 && opCode != 4) param2 = param2Mode == Mode.Position ? x[x[i + 2]] : x[i + 2];
        ...
        /// </summary>        
        [Benchmark]
        public int PartTwo() => PartTwo0(data!, 5);

        private static int PartTwo0(List<string> input, int inputValue)
        {
            int r = 0;
            List<int> x = input[0].Split(',').Select(int.Parse).ToList();
        ...
        #region UnitTest
        public static int PartOne_Test(List<string> data, int inputValue) => PartOne0(data, inputValue);

        public static int PartTwo_Test(List<string> data, int inputValue) => PartTwo0(data, inputValue);
        #endregion

        #region Host
        private readonly IRftAdventOfCodeHostData? RftAdventOfCodeHostData;

        public _05_SunnyWithAChanceOfAsteroids(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_05_SunnyWithAChanceOfAsteroids));
        }
