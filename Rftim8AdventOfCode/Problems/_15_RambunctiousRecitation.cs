...
{
    public class _15_RambunctiousRecitation : I_15_RambunctiousRecitation
    {
        #region Static
        private readonly List<string>? data;

        public _15_RambunctiousRecitation()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_15_RambunctiousRecitation));
        }
        ...
        /// Since it had been spoken before, the next number to speak is the difference between the turn number when it was 
        /// last spoken(the previous turn, 4) and the turn number of the time it was most recently spoken before then(turn 1). 
        ...
        {
            int[] input = input0[0].Split(",").Select(int.Parse).ToArray();

            int limit = 2020;
            int[] spoken = new int[limit];
            Array.Fill(spoken, 0);
        ...
        public _15_RambunctiousRecitation(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_15_RambunctiousRecitation));
        }
