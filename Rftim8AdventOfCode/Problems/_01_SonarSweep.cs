...
{
    public class _01_SonarSweep : I_01_SonarSweep
    {
        #region Static
        ...
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_01_SonarSweep));
        }
        ...
        /// It's covered in Christmas lights(because of course it is), and it even has an experimental antenna that should be able 
        /// to track the keys if you can boost its signal strength high enough; there's a little meter that indicates the antenna's signal strength by displaying 0-50 stars.
        ...
        /// 
        /// The first order of business is to figure out how quickly the depth increases, just so you know what you're dealing with - you never know 
        ...
        private static int PartOne0(List<string> input)
        {
            ...
            {
                if (int.Parse(input[i]) > int.Parse(input[i - 1])) r++;
            }
        ...
        private static int PartTwo0(List<string> input)
        {
            ...
            for (int i = 0; i < input.Count - 3; i++)
            {
                ...
                int c = int.Parse(input[i + 2]);
                int d = int.Parse(input[i + 3]);
        ...
        public _01_SonarSweep(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_01_SonarSweep));
        }
