...
{
    public class _11_MonkeyInTheMiddle : I_11_MonkeyInTheMiddle
    {
        #region Static
        private readonly List<string>? data;

        public _11_MonkeyInTheMiddle()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_11_MonkeyInTheMiddle));
        }
        ...
        /// In this example, the two most active monkeys inspected items 101 and 105 times.The level of monkey business in this situation can be found 
        ...
        private static long PartOne0(List<string> input0)
        {
            ...
            for (int part = 1; part <= 1; part++)
            {
                ...
                int superModulo = monkeys.Aggregate(1, (current, monkey) => current * monkey.Test);
                ...
                for (int i = 0; i < rounds; i++)
                {
                    foreach (Monkey monkey in monkeys)
                    {
                        while (monkey.Items.Count > 0)
                        {
                            ...
                            long worry = part == 1 ? monkey.EvaluateOperation(item) / 3 : monkey.EvaluateOperation(item) % superModulo;
                ...
                r = topMonkeys[0].ProcessedItems * topMonkeys[1].ProcessedItems;
            }
        ...
        public _11_MonkeyInTheMiddle(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_11_MonkeyInTheMiddle));
        }
