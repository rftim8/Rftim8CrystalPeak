...
{
    public class _08_SpaceImageFormat : I_08_SpaceImageFormat
    {
        #region Static
        private readonly List<string>? data;

        public _08_SpaceImageFormat()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_08_SpaceImageFormat));
        }
        ...
        /// 
        /// Images are sent as a series of digits that each represent the color of a single pixel.The digits fill each row of the image left-to-right,
        /// then move downward to the next row, filling rows top-to-bottom until every pixel of the image is filled.
        /// Each image actually consists of a series of identically-sized layers that are filled in this way.So, the first digit corresponds to the top-left pixel of the first layer,
        ...
        private static string PartTwo0(List<string> data)
        {
            return "";
        }
        #endregion
        ...
        public _08_SpaceImageFormat(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_08_SpaceImageFormat));
        }
