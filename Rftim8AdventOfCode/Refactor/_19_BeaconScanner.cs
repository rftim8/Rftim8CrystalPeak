...
{
    public class _19_BeaconScanner : I_19_BeaconScanner
    {
        #region Static
        private readonly List<string>? data;

        public _19_BeaconScanner()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_19_BeaconScanner));
        }

        /// <summary>
        /// As your probe drifted down through this area, it released an assortment of beacons and scanners into the water. 
        /// It's difficult to navigate in the pitch black open waters of the ocean trench, but if you can build a map of the trench using data from the scanners,
        ...
        /// that have overlapping detection regions such that there are at least 12 beacons that both scanners detect within the overlap.
        ...
        ///         Or, two scanners might agree on which direction is positive x, but one scanner might be upside-down from the perspective of the other scanner.
        ...
        {
            List<Vector3[]> scans = string.Join(" ", input)
                .Split("  ")
                .Select(s => s.Split("--- ")
                .Last()
                .Split(" ")
                ...
                .ToArray())
                .ToList();
            Vector3[] system = [.. scans[0]];
            scans.RemoveAt(0);


            List<Vector3> scanners = [new Vector3(0, 0, 0)];
            do
            {
                IEnumerable<(Vector3[] match, Vector3[] translated, Vector3 offset)> linked = from match in scans
                ...
                (Vector3[] match, Vector3[] translated, Vector3 offset) link = linked.First();
        ...
        private static IEnumerable<float> distances = [];

        private static Vector3 Rotate(Vector3 v, int direction)
        {
            (float, float, float) rotated = direction switch
            {
                0 => (v.X, v.Y, v.Z),
                1 => (v.Y, v.Z, v.X),
                2 => (-v.Y, v.X, v.Z),
        ...
        public _19_BeaconScanner(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_19_BeaconScanner));
        }
