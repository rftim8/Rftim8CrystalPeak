...
{
    public class _12_HotSprings : I_12_HotSprings
    {
        #region Static
        private readonly List<string>? data;

        public _12_HotSprings()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_12_HotSprings));
        }
        ...
        /// and each number is the entire size of its contiguous group (that is, groups are always separated by at least
        ...
        /// and three broken springs (in that order) can appear in that row: the first three unknown springs must be broken, 
        ...
        /// if that were true, the numbers afterward would have been 2,3 instead.) Since each ?? can either be #. or .#,
        ...
        /// However, the remaining run of unknown spring conditions have many different ways they could 
        ...
        private static long PartOne0(List<string> input)
        {
            ...
            foreach (string item in input)
            {
                ...
                int[] y = [.. item.Split(" ")[1].Split(',').Select(int.Parse)];
                r += Ways(x, y);
            }
        ...
        /// of itself(separated by ?) and replace the list of contiguous groups of damaged springs with five copies of itself(separated by,).
        /// 
        /// So, this row:
        /// 
        /// .# 1
        ...
        private static long PartTwo0(List<string> input)
        {
            ...
            foreach (string item in input)
            {
                ...
                x += $"{t}";
                int[] y = [.. item.Split(" ")[1].Split(',').Select(int.Parse)];
                int[] z = [.. Enumerable.Repeat(y, 5).SelectMany(o => o)];
                r += Ways(x, z);
            }
        ...
        private static long Ways(string pattern, int[] counts) => Ways(pattern, counts, 0, 0, []);

        private static long Ways(string pattern, int[] counts, int patternIndex, int countIndex, Dictionary<(int, int), long> waysMemo)
        {
            (int patternIndex, int countIndex) memoKey = (patternIndex, countIndex);

            if (waysMemo.TryGetValue(memoKey, out long memo)) return memo;

            if (patternIndex >= pattern.Length) return countIndex == counts.Length ? 1 : 0;

            long ways = 0;
            char currentPatternChar = pattern[patternIndex];
            if (currentPatternChar != '#') ways += Ways(pattern, counts, patternIndex + 1, countIndex, waysMemo);

            if (currentPatternChar != '.' && countIndex < counts.Length)
            {
                bool enoughToConsume = true;
                int neededCount = counts[countIndex] - 1;
                ...
                if (enoughToConsume)
                {
                    int separatorIndex = patternIndex + 1;
                    if (separatorIndex >= pattern.Length || pattern[separatorIndex] != '#') ways += Ways(pattern, counts, separatorIndex + 1, countIndex + 1, waysMemo);
                }
            }
        ...
        public _12_HotSprings(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_12_HotSprings));
        }
