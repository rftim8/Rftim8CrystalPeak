...
{
    public class _01_InverseCaptcha : I_01_InverseCaptcha
    {
        #region Static
        private readonly List<string>? data;

        public _01_InverseCaptcha()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_01_InverseCaptcha));
        }

        /// <summary>
        /// The night before Christmas, one of Santa's Elves calls you in a panic. 
        /// "The printer's broken! We can't print the Naughty or Nice List!" By the time you make it to sub-basement 17, there are only a few minutes until midnight. 
        /// "We have a big problem," she says; "there must be almost fifty bugs in this system, but nothing else can print The List. 
        /// Stand in this square, quick! There's no time to explain; if you can convince them to pay you in stars, you'll be able to--" She pulls a lever and the world goes blurry.
        /// 
        ...
        private static int PartOne0(List<string> input)
        {
            ...
            for (int i = 1; i < n; i++)
            {
                if (input[0][i] == input[0][i - 1]) r += int.Parse(input[0][i].ToString());

                if (i == n - 1)
                {
                    if (input[0][i] == input[0][0]) r += int.Parse(input[0][i].ToString());
                }
            }
        ...
        private static int PartTwo0(List<string> input)
        {
            ...
            int n = input[0].Length;
            int m = n / 2;
            string t = input[0] + input[0];

            for (int i = 0; i < n; i++)
            {
                if (t[i] == t[i + m]) r += int.Parse(t[i].ToString());
            }
        ...
        public _01_InverseCaptcha(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_01_InverseCaptcha));
        }
