...
{
    public class _02_CubeConundrum : I_02_CubeConundrum
    {
        #region Static
        private readonly List<string>? data;

        public _02_CubeConundrum()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_02_CubeConundrum));
        }
        ...
        /// Each game is listed with its ID number(like the 11 in Game 11: ...) followed by a semicolon-separated list of subsets of cubes that were revealed from 
        ...
        private static int PartOne0(List<string> input)
        {
            ...
            {
                int a = int.Parse(item.Split(":")[0].Split(" ")[1]);
                bool f = true;

                List<string> b = [.. item.Split(":")[1].Split(";", StringSplitOptions.RemoveEmptyEntries)];

                foreach (string item1 in b)
                {
                    List<string> c = [.. item1.Split(",", StringSplitOptions.RemoveEmptyEntries)];

                    foreach (string item2 in c)
                    {
                        if (item2.Contains("red") && int.Parse(item2.Split(" ", StringSplitOptions.RemoveEmptyEntries)[0].Trim()) > 12) f = false;
                        if (item2.Contains("green") && int.Parse(item2.Split(" ", StringSplitOptions.RemoveEmptyEntries)[0].Trim()) > 13) f = false;
                        if (item2.Contains("blue") && int.Parse(item2.Split(" ", StringSplitOptions.RemoveEmptyEntries)[0].Trim()) > 14) f = false;
                    }
                }
        ...
        private static int PartTwo0(List<string> input)
        {
            ...
            foreach (string item in input)
            {
                ...
                foreach (string item1 in b)
                {
                    ...
                    foreach (string item2 in c)
                    {
                        if (item2.Contains("red")) red.Add(item2);
                        if (item2.Contains("green")) green.Add(item2);
                        if (item2.Contains("blue")) blue.Add(item2);
                    }
                }

                int red0 = int.Parse(red.MaxBy(o => int.Parse(o.Split(" ", StringSplitOptions.RemoveEmptyEntries)[0].Trim()))!.Split(" ", StringSplitOptions.RemoveEmptyEntries)[0]);
                int green0 = int.Parse(green.MaxBy(o => int.Parse(o.Split(" ", StringSplitOptions.RemoveEmptyEntries)[0].Trim()))!.Split(" ", StringSplitOptions.RemoveEmptyEntries)[0]);
                int blue0 = int.Parse(blue.MaxBy(o => int.Parse(o.Split(" ", StringSplitOptions.RemoveEmptyEntries)[0].Trim()))!.Split(" ", StringSplitOptions.RemoveEmptyEntries)[0]);
        ...
        public _02_CubeConundrum(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_02_CubeConundrum));
        }
