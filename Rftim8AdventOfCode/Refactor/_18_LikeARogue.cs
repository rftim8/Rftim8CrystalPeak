...
{
    public class _18_LikeARogue : I_18_LikeARogue
    {
        #region Static
        private readonly List<string>? data;

        public _18_LikeARogue()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_18_LikeARogue));
        }
        ...
        /// Then, we continue on to the next row, using the same rules, and get ^^..^. After determining two new rows, our map looks like this:
        /// 
        /// ..^^.
        /// .^^^^
        /// ^^..^
        ...
        private static int PartOne0(List<string> input)
        {
            ...
            {
                if (s[index] == '.') tileMap[0].Add(true);
                else tileMap[0].Add(false);
            }
            ...
            for (int row = 1; row < rows; row++)
            {
                ...
                {
                    if (tileMap[row - 1][column - 1] == tileMap[row - 1][column] && tileMap[row - 1][column - 1] != tileMap[row - 1][column + 1]) tileMap[row].Add(false);
                    else if (tileMap[row - 1][column - 1] != tileMap[row - 1][column] && tileMap[row - 1][column] == tileMap[row - 1][column + 1]) tileMap[row].Add(false);
                    else tileMap[row].Add(true);
                }
            ...
            for (int row = 0; row < rows; row++)
            {
                ...
                {
                    if (tileMap[row][column]) safeCounter++;
                }
            }
        ...
        public _18_LikeARogue(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_18_LikeARogue));
        }
