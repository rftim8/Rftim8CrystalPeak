...
{
    public class _17_TrickShot : I_17_TrickShot
    {
        #region Static
        private readonly List<string>? data;

        public _17_TrickShot()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_17_TrickShot));
        }
        ...
        /// This target area means that you need to find initial x,y velocity values such that after any step, the probe's x position is at least 20 and at most 30, 
        ...
        {
            int[][] target = input[0].Split(',')
                .Select(s => s.Split('=')[1])
                .Select(s => s.Split("..").Select(int.Parse).ToArray())
                .ToArray();
            IEnumerable<int> possibleXs = Enumerable.Range((int)Math.Sqrt(target[0][0]), target[0][1]);
            IEnumerable<int> possibleYs = Enumerable.Range(target[1][0], -2 * target[1][0]);
            IEnumerable<(int x, int y)> possibleVs = possibleXs.SelectMany(x => possibleYs.Select(y => (x, y)));

            int?[] maxYsWhereHits = possibleVs.Select(v => MaxYIfHits(v, target))
            .Where(maxY => maxY.HasValue)
            .ToArray();
            return maxYsWhereHits.Max();
        }
        ...
        private static int PartTwo0(List<string> input)
        {
            ...
            return maxYsWhereHits.Length;
        }

        private static int? MaxYIfHits((int x, int y) v, int[][] target)
        {
            for (
            ...
            {
                if (x >= target[0][0] && x <= target[0][1] && y >= target[1][0] && y <= target[1][1]) return maxY;
            }
        ...
        public _17_TrickShot(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_17_TrickShot));
        }
