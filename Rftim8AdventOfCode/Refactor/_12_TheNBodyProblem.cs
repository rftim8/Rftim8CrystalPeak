...
using State = System.UInt64;

namespace Rftim8AdventOfCode
{
    public partial class _12_TheNBodyProblem : I_12_TheNBodyProblem
    {
        #region Static
        private readonly List<string>? data;

        public _12_TheNBodyProblem()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_12_TheNBodyProblem));
        }

        /// <summary>
        /// The space near Jupiter is not a very safe place; you need to be careful of a big distracting red spot, extreme radiation,
        /// and a whole lot of moons swirling around. You decide to start by tracking the four largest moons: Io, Europa, Ganymede, and Callisto.
        /// 
        /// After a brief scan, you calculate the position of each moon(your puzzle input). You just need to simulate their motion so you can avoid them.
        /// 
        /// Each moon has a 3-dimensional position (x, y, and z) and a 3-dimensional velocity. The position of each moon is given in your scan;
        ...
        /// if Europa has a position of x = 1, y= 2, z= 3 and a velocity of x= -2, y= 0, z= 3, then its new position would be x = -1, y = 2, z = 6.
        ...
        private static int PartOne0(List<string> input0)
        {
            ...
            foreach (string item in input0)
            {
                foreach ((int x, int y, int z) in from Match match in re.Matches(item)
            ...
            List<Coord> xCoords = Copy(input[0]);
            List<Coord> yCoords = Copy(input[1]);
            List<Coord> zCoords = Copy(input[2]);
            ...
            return TotalEnergy(xCoords, yCoords, zCoords);
        }
        ...
        public static int Sign(int x) => x > 0 ? 1 : x < 0 ? -1 : 0;

        public static int Abs(int x) => x >= 0 ? x : -x;

        private static void Step(List<Coord> coords)
        {
        ...
        private static int TotalEnergy(List<Coord> xCoords, List<Coord> yCoords, List<Coord> zCoords)
        {
            int energy = 0;

            for (int i = 0; i < xCoords.Count; i++)
            {
                int pot = Abs(xCoords[i].pos) + Abs(yCoords[i].pos) + Abs(zCoords[i].pos);
                int kin = Abs(xCoords[i].v) + Abs(yCoords[i].v) + Abs(zCoords[i].v);
                energy += pot * kin;
            }
        ...
        private static List<Coord> Copy(List<Coord> input)
        {
            List<Coord> result = [];

            foreach (Coord c in input)
            {
                result.Add(new Coord(c));
            }

            return result;
        }

        private static State ToState(List<Coord> coords) => (byte)(State)(coords[0].pos & 0xff) |
                    (State)(coords[0].v & 0xff) << 8 |
                    (State)(coords[1].pos & 0xff) << 16 |
                    (State)(coords[1].v & 0xff) << 24 |
                    (State)(coords[2].pos & 0xff) << 32 |
                    (State)(coords[2].v & 0xff) << 40 |
                    (State)(coords[3].pos & 0xff) << 48 |
                    (State)(coords[3].v & 0xff) << 56;

        private static Tuple<long, long> FindCycle(List<Coord> coords)
        {
            Dictionary<State, long> record = [];
            long step = 0;

            while (true)
            {
                State state = ToState(coords);
            
                if (record.TryGetValue(state, out long value))
                {
                    long prev = value;

                    return new Tuple<long, long>(prev, step - prev);
                }
        ...
        public _12_TheNBodyProblem(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_12_TheNBodyProblem));
        }
