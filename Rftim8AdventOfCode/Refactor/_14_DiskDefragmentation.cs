...
{
    public class _14_DiskDefragmentation : I_14_DiskDefragmentation
    {
        #region Static
        private readonly List<string>? data;

        public _14_DiskDefragmentation()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_14_DiskDefragmentation));
        }
        ...
        /// each hash contains 128 bits which correspond to individual grid squares.Each bit of a hash indicates whether that square is free(0) or used(1).
        /// 
        ...
        /// 0 becomes 0000, 1 becomes 0001, e becomes 1110, f becomes 1111, and so on; a hash that begins with 
        ...
        /// all of the squares marked 8 are connected when considering the whole 128x128 grid.In total, in this example, 1242 regions are present.
        ...
        public _14_DiskDefragmentation(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_14_DiskDefragmentation));
        }
