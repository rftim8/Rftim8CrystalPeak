...
{
    public class _15_BeverageBandits : I_15_BeverageBandits
    {
        #region Static
        private readonly List<string>? data;

        public _15_BeverageBandits()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_15_BeverageBandits));
        }
        ...
        /// regardless of the type of unit or whether other units have moved after the round started. For example:
        /// 
        /// would take their
        ...
        /// these are the squares which are adjacent(immediately up, down, left, or right) to any target and which aren't already occupied by a wall or another unit.
        /// Alternatively, the unit might already be in range of a target. If the unit is not already in range of a target,
        ...
        /// A step is a single movement to any adjacent(immediately up, down, left, or right) open(.) square.Units cannot move into walls or other units.
        ...
        /// the two squares to which the Elf could move on this turn(down and to the right) are both equally good moves and would leave the Elf 
        /// 2 steps from being in range of the Goblin.Because the step which is first in reading order is chosen, the Elf moves right one square.
        /// 
        ...
        /// #.......#
        /// #########
        /// Once the Goblins and Elf reach the positions above, they all are either in range of a target or cannot find any square in range of a target,
        ...
        /// ..G..  2       ..G..  2  
        /// ...G.  1       ...G.  1  
        /// The "HP" column shows the hit points of the Goblin to the left in the corresponding row.The Elf is in range of three targets:
        /// the Goblin above it (with 4 hit points), the Goblin to its right (with 2 hit points), and the Goblin below it(also with 2 hit points).
        ...
        /// After attacking, the unit's turn ends. Regardless of how the unit's turn ends, the next unit in the round takes its turn.
        ...
        /// the number of full rounds that were completed (not counting the round in which combat ends) multiplied by the sum of the hit points of all 
        ...
        private static int PartOne0(List<string> input)
        {
            ...
            return new Game(map, 3).RunGame(false) ?? 0;
        }
        ...
        private class Game
        {
            ...
            public Game(string[] initialMap, int elfAttackPower)
            {
                for (int y = 0; y < initialMap.Length; y++)
                {
                    ...
                    {
                        if (initialMap[y][x] == 'G') _units.Add(
                            new Unit
                            {
                                X = x,
                                Y = y,
                        ...
                        else if (initialMap[y][x] == 'E') _units.Add(
                            new Unit
                            {
                                X = x,
                                Y = y,
            ...
            public int? RunGame(bool failOnElfDeath)
            {
                ...
                {
                    _units = [.. _units.OrderBy(u => u.Y).ThenBy(u => u.X)];
                    for (int i = 0; i < _units.Count; i++)
                    {
                        Unit u = _units[i];
                        List<Unit> targets = _units.Where(t => t.IsGoblin != u.IsGoblin).ToList();

                        if (targets.Count == 0) return rounds * _units.Sum(ru => ru.Health);

                        if (!targets.Any(t => IsAdjacent(u, t))) TryMove(u, targets);

                        Unit? bestAdjacent = targets
                        ...
                        if (bestAdjacent == null) continue;

                        bestAdjacent.Health -= u.AttackPower;
                        if (bestAdjacent.Health > 0) continue;

                        if (failOnElfDeath && !bestAdjacent.IsGoblin) return null;
            ...
            {
                HashSet<(int x, int y)> inRange = [];
                foreach (Unit target in targets)
                {
                    foreach ((int dx, int dy) in s_neis)
                    {
                        (int nx, int ny) = (target.X + dx, target.Y + dy);
                        if (IsOpen(nx, ny)) inRange.Add((nx, ny));
                    }
                }

                Queue<(int x, int y)> queue = new();
                Dictionary<(int x, int y), (int px, int py)> prevs = [];
                ...
                while (queue.Count > 0)
                {
                    ...
                    foreach ((int dx, int dy) in s_neis)
                    {
                        ...
                        if (prevs.ContainsKey(nei) || !IsOpen(nei.x, nei.y)) continue;
                ...
                {
                    if (!prevs.ContainsKey((destX, destY))) return null;
                ...
                List<(int tx, int ty, List<(int x, int y)>? path)> paths = [.. inRange
                ...
                if (bestPath != null) (u.X, u.Y) = bestPath[0];
            }
        ...
        public _15_BeverageBandits(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_15_BeverageBandits));
        }
