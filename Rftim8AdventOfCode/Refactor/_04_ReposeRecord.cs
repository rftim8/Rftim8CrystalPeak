...
{
    public class _04_ReposeRecord : I_04_ReposeRecord
    {
        #region Static
        private readonly List<string>? data;

        public _04_ReposeRecord()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_04_ReposeRecord));
        }
        ...
        /// They've been writing down the ID of the one guard on duty that night - the Elves seem to have decided that one guard was enough for the overnight shift - 
        ...
        {
            Dictionary<int, List<int>> guards = [];
            input = [.. input.OrderBy(o => o.Split(']')[0])];
            int crtId = 0;
            string crtFall = string.Empty;
            foreach (string item in input)
            {
                ...
                {
                    case "Guard":
                        if (int.TryParse(item.Split(' ')[3].Replace("#", ""), out int id))
                        {
                            if (!guards.ContainsKey(id)) guards.Add(id, []);
                            crtId = id;
                        }
                        break;
                    case "falls":
                        crtFall = item.Split(']')[0].Split(' ')[1];
                        break;
                    case "wakes":
                        string crtWake = item.Split(']')[0].Split(' ')[1];

                        if (crtFall.Split(':')[0] == "23" && crtWake.Split(':')[0] == "23" ||
                            crtFall.Split(':')[0] == "00" && crtWake.Split(':')[0] == "00")
                        {
                            int i = int.Parse(crtFall.Split(':')[1]);
                            int j = int.Parse(crtWake.Split(':')[1]);
                            for (int k = i; k <= j; k++)
                            {
                                guards[crtId].Add(k);
                            }
            ...
            KeyValuePair<int, List<int>> x = guards.MaxBy(o => o.Value.Count);
            int max = x.Value.GroupBy(o => o).OrderByDescending(o => o.Count()).First().Key;
        ...
        private static int PartTwo0(List<string> input)
        {
            ...
            foreach (KeyValuePair<int, List<int>> item in guards)
            {
                ...
                {
                    int t = item.Value.GroupBy(o => o).OrderByDescending(o => o.Count()).First().Key;
                    int t0 = item.Value.Where(o => o == t).Count();
                    if (c < t0)
                    {
                        c = t0;
                        r = item.Key;
        ...
        public _04_ReposeRecord(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_04_ReposeRecord));
        }
