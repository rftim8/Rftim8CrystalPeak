...
{
    public class _10_AdapterArray : I_10_AdapterArray
    {
        #region Static
        private readonly List<string>? data;

        public _10_AdapterArray()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_10_AdapterArray));
        }
        ...
        ///
        ///
        /// The charging outlet has an effective rating of 0 jolts, so the only adapters that could connect to it directly would need 
        /// to have a joltage rating of 1, 2, or 3 jolts.Of these, only one you have is an adapter rated 1 jolt (difference of 1).
        ...
        {
            int[] jolts = [0, .. input.Select(int.Parse).OrderBy(j => j)];
            int device = jolts.Max() + 3;
        ...
        private static long PartTwo0(List<string> input)
        {
            ...
            Dictionary<int, long> routesFromJolts = new() { { device, 1 } };
        ...
        public _10_AdapterArray(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_10_AdapterArray));
        }
