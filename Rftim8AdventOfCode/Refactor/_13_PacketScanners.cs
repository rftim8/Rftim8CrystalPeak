...
{
    public class _13_PacketScanners : I_13_PacketScanners
    {
        #region Static
        private readonly List<string>? data;

        public _13_PacketScanners()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_13_PacketScanners));
        }
        ...
        /// [ ]
        /// 
        /// Within each layer, a security scanner moves back and forth within its range.Each security scanner starts at the top and moves down until it reaches the bottom,
        ...
        /// [S]     [S]
        /// 
        /// Your plan is to hitch a ride on a packet about to move through the firewall.The packet will travel along the top of each layer,
        /// and it moves at one layer per picosecond. Each picosecond, the packet moves one layer forward (its first move takes it into layer 0), 
        ...
        {
            List<List<int>> r = input.Select(o => o.Split(':').Select(t => int.Parse(t)).ToList()).ToList();

            return r.Sum(s => s[0] % (s[1] * 2 - 2) == 0 ? s[0] * s[1] : 0);
        }
        ...
        private static int PartTwo0(List<string> input)
        {
            ...
            return Enumerable.Range(0, int.MaxValue).First(d => r.All(s => (s[0] + d) % (s[1] * 2 - 2) != 0));
        }
        #endregion
        ...
        public _13_PacketScanners(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_13_PacketScanners));
        }
