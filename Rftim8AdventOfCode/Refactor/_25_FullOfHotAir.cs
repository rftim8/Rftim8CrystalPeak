...
{
    public class _25_FullOfHotAir : I_25_FullOfHotAir
    {
        #region Static
        private readonly List<string>? data;

        public _25_FullOfHotAir()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_25_FullOfHotAir));
        }
        ...
        /// 
        /// The fuel heating machine is a new addition to the process.When this mountain was a volcano, the ambient temperature was more reasonable; 
        ...
        /// 
        /// "That's right, just supply the fuel amount to the-- oh, for more than one burner? No problem, you just need to add together our Special Numeral-Analogue Fuel Units.
        ...
        /// You know how starting on the right, normal numbers have a ones place, a tens place, a hundreds place, and so on, where the digit in each place 
        /// tells you how many of that value you have?"
        /// 
        /// "SNAFU works the same way, except it uses powers of five instead of ten. 
        /// Starting from the right, you have a ones place, a fives place, a twenty-fives place, a one-hundred-and-twenty-fives place, and so on. It's that easy!"
        /// 
        ...
        /// Instead, you are met with buttons labeled =, -, 0, 1, and 2. Bob needs the input value expressed as a SNAFU number, not in decimal.
        /// 
        ...
        private static string PartOne0(List<string> input)
        {
            ...
            {
                total += SnafuToLong(item);

            }
            Console.WriteLine($"4890: {LongToSnafu(4890)}");
        ...
        private static long SnafuToLong(string snafu)
        {
            ...
            {
                long place = snafu.Length - i - 1;
                long num = snafu[i] switch
                {
                    '-' => -1,
                    '=' => -2,
                    var x => long.Parse(x.ToString())
                };
                val += (long)(num * Math.Pow(5, place));
            }

            return val;
        }

        private static string LongToSnafu(long val)
        {
        ...
        public _25_FullOfHotAir(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_25_FullOfHotAir));
        }
