...
{
    public class _06_UniversalOrbitMap : I_06_UniversalOrbitMap
    {
        #region Static
        private readonly List<string>? data;

        public _06_UniversalOrbitMap()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_06_UniversalOrbitMap));
        }
        ...
        /// 
        /// Whenever A orbits B and B orbits C, then A indirectly orbits C. This chain can be any number of objects long: if A orbits B, B orbits C, 
        ...
        {
            Dictionary<string, string> orbits = input.ToDictionary(l => l.Split(')')[1], l => l.Split(')')[0]);
            List<string> GetParents(string obj)
            {
                List<string> result = [];
                for (string curr = obj; curr != "COM"; curr = orbits[curr])
                {
                    result.Add(curr);
                }
            ...
            return orbits.Keys.Sum(obj => GetParents(obj).Count - 1);
        }
        ...
        public _06_UniversalOrbitMap(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_06_UniversalOrbitMap));
        }
