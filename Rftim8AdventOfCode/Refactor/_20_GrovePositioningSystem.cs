...
{
    public class _20_GrovePositioningSystem : I_20_GrovePositioningSystem
    {
        #region Static
        private readonly List<string>? data;

        public _20_GrovePositioningSystem()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_20_GrovePositioningSystem));
        }
        ...
        /// 
        /// The encrypted file is a list of numbers. To mix the file, move each number forward or backward in the file a number of positions 
        /// equal to the value of the number being moved.The list is circular, so moving a number off one end of the list wraps back around 
        ...
        {
            List<(int Index, long Value)> initialList = GetInitialFile(input, 1);
            return GetEncryptedCoordinates(initialList, 1);
        }
        ...
        {
            List<(int Index, long Value)> initialList = GetInitialFile(input, 811589153);

            return GetEncryptedCoordinates(initialList, 10);
        }

        private static List<(int Index, long Value)> GetInitialFile(List<string> input, long decryptionKey)
        {
            return input.Select((i, index) => (index, long.Parse(i) * decryptionKey)).ToList();
        }

        private static long GetEncryptedCoordinates(List<(int Index, long Value)> initialList, int numberOfMixes)
        {
            List<(int Index, long Value)> numbers = new(initialList);
            int length = numbers.Count - 1;
            for (int i = 0; i < numberOfMixes; ++i)
            {
                foreach ((int index, long value) in initialList)
                {
                    int pos = numbers.IndexOf((index, value));
                    int newPos = (int)((pos + value) % length);
            ...
            int zeroPos = numbers.FindIndex(i => i.Value == 0);

            return numbers[(zeroPos + 1000) % numbers.Count].Value +
                   numbers[(zeroPos + 2000) % numbers.Count].Value +
                   numbers[(zeroPos + 3000) % numbers.Count].Value;
        }
        #endregion
        ...
        public _20_GrovePositioningSystem(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_20_GrovePositioningSystem));
        }
