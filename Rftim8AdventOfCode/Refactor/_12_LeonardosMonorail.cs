...
{
    public class _12_LeonardosMonorail : I_12_LeonardosMonorail
    {
        #region Static
        private readonly List<string>? data;

        public _12_LeonardosMonorail()
        {
            data = RftAdventOfCodeStaticData.Input_Test(testType: false, problemName: nameof(_12_LeonardosMonorail));
        }
        ...
        /// dec a
        /// 
        /// The above code would set register a to 41, increase its value by 2, decrease its value by 1, and then skip the last dec a (because a is not zero, so the jnz a 2 skips it), 
        ...
        private static int PartOne0(List<string> input)
        {
            ...
            for (int i = 0; i < input.Count; i++)
            {
                ...
                if (n == 2)
                {
                    if (command == "inc") kvp[input[i].Split(' ')[1]]++;
                    else if (command == "dec") kvp[input[i].Split(' ')[1]]--;
                }
                else
                {
                    if (command == "cpy")
                    {
                        if (int.TryParse(input[i].Split(' ')[1], out int value))
                        {
                            kvp[input[i].Split(' ')[2]] = value;
                        }
                        else
                        {
                            kvp[input[i].Split(' ')[2]] = kvp[input[i].Split(' ')[1]];
                        }
                    }
                    else
                    {
        ...
        public _12_LeonardosMonorail(IHost host)
        {
            RftAdventOfCodeHostData = host.Services.GetRequiredService<IRftAdventOfCodeHostData>();
            data = RftAdventOfCodeHostData.Input_Test(problemName: nameof(_12_LeonardosMonorail));
        }
